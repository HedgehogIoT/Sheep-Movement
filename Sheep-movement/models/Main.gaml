...
global {
	...
	reflex update_reached_count {
		sheep_reached <- length(sheep where each.reached_target);
	}
...
species sheep skills: [moving] {
	...
	reflex check_target {
		...
		if (!reached_target) {
			if (location distance_to first(shepherds).location < 2.0) {
				reached_target <- true;
				reach_time <- cycle;
			}
		}
...
experiment "chart" type: gui {
	output {
		display "Sheep reach time" type: 2d {

	chart "Cumulative sheep reaching target"
		type: series
		x_label: "Time (cycles)"
		y_label: "Number of sheep"
		color: #red
		background: #white {
